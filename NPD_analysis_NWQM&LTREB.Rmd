...
temp_data_plot = temp %>%
  group_by(year_season) %>%
  mutate(effFlux_byyear_season = mean(effFlux_r2, na.rm = T),
         deltaHL_byyear_season = mean(deltaHL_r2,na.rm = T)) %>%
  distinct(.,.keep_all=T)

ggplot(temp_data_plot, aes(x=deltaHL_byyear_season, y=effFlux_byyear_season,colour=rest_treat)) + 
  geom_point(size=3) +
  facet_grid(~ season)
